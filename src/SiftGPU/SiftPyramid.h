...
public:
	...
	enum {
		...
		TRUNCATE_METHOD_KEEP_HIGHEST_LEVELS_0 = 0, // -tc, -tc1, keep the highest levels
		TRUNCATE_METHOD_KEEP_HIGHEST_LEVELS_1 = 1, // -tc2       keep the highest level (should be faster than -tc)
		TRUNCATE_METHOD_KEEP_LOWEST_LEVELS    = 2, // -tc3       keep the lowest levels
#ifdef TOP_K_SELECTION
		TRUNCATE_METHOD_TOP_K                 = 3, // -topk      top k selection
...
#define SIFTGPU_ENABLE_REVERSE_ORDER
...
#ifdef GPU_HESSIAN
	#define FIRST_LEVEL(R)             (R ? param._dog_level_num : 1)   
	...
	#ifdef GPU_HESSIAN
		#define FOR_EACH_LEVEL(j, R)  for(int j = 1; j <= param._dog_level_num; ++j)
	#else
