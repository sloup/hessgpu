...
void ShaderBagGLSL::LoadDisplayShaders()
{
  ...
  s_display_gaussian =  new ProgramGLSL(
    "uniform sampler2DRect tex;\n"
	"void main(void) {\n"
    "  float r = texture2DRect(tex, gl_TexCoord[0].st).r;\n"
  ...
  s_display_dog =  new ProgramGLSL(
    "uniform sampler2DRect tex;\n"
    "void main(void){\n"
    "  float g = texture2DRect(tex, gl_TexCoord[0].st).g;\n"
  ...
  s_display_dog =  new ProgramGLSL(
    "uniform sampler2DRect tex;\n"
    "void main(void){\n"
    "  float g = 0.5+(20.0*texture2DRect(tex, gl_TexCoord[0].st).g);\n"
    "  gl_FragColor = vec4(g, g, g, 0.0);\n"
    "}"
  );
#endif // GPU_HESSIAN
  ...
  s_display_grad = new ProgramGLSL(
    ...
    "void main(void) {\n"
    "  vec4 cc = texture2DRect(tex, gl_TexCoord[0].st);\n"
    "  gl_FragColor = vec4(5.0*cc.bbb, 1.0);\n"
    "}"
  );
  ...
  s_display_keys= new ProgramGLSL(
    "uniform sampler2DRect tex;\n"
    "void main(void) {\n"
    "  vec4 keyColor = texture2DRect(tex, gl_TexCoord[0].st);\n"
	"  if(keyColor.a < 0.5)\n"
	"    discard;\n"
  ...
  s_display_keys= new ProgramGLSL(
    ...
    "  vec4 cc = texture2DRect(tex, gl_TexCoord[0].st);\n"
    "  if(cc.r == 0.0) discard;\n"
    "  gl_FragColor = ((cc.r == 1.0) ? vec4(1.0, 0.0, 0,1.0) : vec4(0.0,1.0,0.0,1.0));\n"
    "}"
  );
#endif // GPU_HESSIAN
}
...
void ShaderBagGLSL::LoadOrientationShader()
{
	...
	"#define ORIENTATION_THRESHOLD "<< GlobalUtil::_MultiOrientationThreshold << "\n"
	"uniform sampler2DRect tex;					\n"
