...
int GlobalParam::		_KeepShaderLoop = 0;
 ...
 int GlobalParam::		_AutoImageDownScaling = 0;
 // size of the measurement region (as multiple of the feature scale)
 float GlobalParam::	_mrSize = 3.0f;
 ...
 bool GlobalParam::     _HalfSIFT = false;
#endif // GPU_HESSIAN
...
float GlobalParam::     _MultiOrientationThreshold = 0.8f;
///
...
double ClockTimer::ClockMS()
{
#if defined(_WIN32)

#if defined(TIMING_BY_CLOCK)
  return clock() * 1000.0 / CLOCKS_PER_SEC;
#else // defined(TIMING_BY_CLOCK)
  ...
  if(started == 0)
  {
    LARGE_INTEGER timerRes; // ticks per second
    QueryPerformanceFrequency(&timerRes);
    ticksPerUS = timerRes.QuadPart / 1000000.0;
  }

  LARGE_INTEGER currentTime;
  QueryPerformanceCounter(&currentTime);
  double timeInUS = currentTime.QuadPart / ticksPerUS; // microseconds
...
#endif // HPU_HESSIAN || GPU_SIFT_MODIFIED

#endif // defined(TIMING_BY_CLOCK)
  ...
  else
  {
    ...
    return (now.tv_usec - tstart.tv_usec + (now.tv_sec - tstart.tv_sec) * 1000000)/1000.0;
  }
...
{
  return ClockMS() * 0.001; // to milliseconds
}
...
void ClockTimer::StopTimer(int verb)
{
  ...
  if(verb && GlobalUtil::_verbose)
  {
    std::cout << "[" << _current_event << "]:\t" << GetElapsedTime() << "ms \n";
  }
}

float ClockTimer::GetElapsedTime()
{
  return (float)((_time_stop - _time_start) * 0.001f); // to milliseconds
}
...
void GlobalUtil::InitGLParam(int NotTargetGL)
{
  ...
  else
  {
    ...
    if(glewGetExtension("GL_ARB_fragment_shader") != GL_TRUE || glewGetExtension("GL_ARB_shader_objects") != GL_TRUE || glewGetExtension("GL_ARB_shading_language_100") != GL_TRUE)
    {
...
// half float: sign 1b | exponent 5b | mantissa 10b
//      float: sign 1b | exponent 8b | mantissa 23b
unsigned int half2float(unsigned short h)
{
  ...
  switch (h_exp)
  {
    ...
    case 0x7c00u: /* inf or NaN */
      ...
      return f_sgn + 0x7f800000u + (((unsigned int)(h & 0x03ffu)) << 13);
    default: /* normalized */
      /* Just need to adjust the exponent and shift */
      return f_sgn + (((unsigned int)(h & 0x7fffu) + 0x1c000u) << 13);
  }
}
