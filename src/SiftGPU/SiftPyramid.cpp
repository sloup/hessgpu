...
void SiftPyramid::RunSIFT(GLTexInput *input)
{
...
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
  _imageWidth = input->GetImgWidth();
  _imageHeight = input->GetImgHeight();
#endif // GPU_HESSIAN
  ...
  else
  {
    ...
    GlobalUtil::StartTimer("Features count reduction");
   // topk computation goes here as well as other feature reduction methods
#endif

    // reduce amount of keypoints (if enabled using "-tc[1|2|3]" or "-topk" options)
    LimitFeatureCount(0);

#if (defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED) && defined TOP_K_SELECTION
    GlobalUtil::StopTimer();
    _timing[8] = GetElapsedTime();
#endif // GPU_HESSIAN
  }
...
void SiftPyramid::LimitFeatureCount(int have_keylist)
{
  ...
  if(GlobalUtil::_TruncateMethod == TRUNCATE_METHOD_TOP_K)
  {
    if(have_keylist)
      return;

    // topk selection goes here
    SelectTopK();
  ...
  else if(GlobalUtil::_TruncateMethod == TRUNCATE_METHOD_KEEP_LOWEST_LEVELS)
#else
  if(GlobalUtil::_TruncateMethod == 2)
#endif // (GPU_HESSIAN || GPU_SIFT_MODIFIED) && TOP_K_SELECTION
  {
    // small scale features are kept while large scale features are discarded
    // features are discarded by the whole levels (large scale feature levels are discarded first)
    int i = 0;
    int new_feature_num = 0;
    int level_num = param._dog_level_num * _octave_num;
    ...
    // _keypoint_buffer is not modified, we just change number of features
  }
  else
  {
    // large scale features are kept while small scale features are discarded
    // features are discarded by the whole levels (small scale feature levels are discarded first)
    ...
    {
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
      _keypoint_buffer.erase(_keypoint_buffer.begin(), _keypoint_buffer.begin() + num_to_erase * SIFT_KEYPOINT_ITEMS);
#else
...
void SiftPyramid::PrepareBuffer()
{
  ...
  // when an existing keypoint list does not have orientation, we need to download them again.
  if(!(_existing_keypoints & SIFT_SKIP_ORIENTATION))
  {
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
    _keypoint_buffer.resize(SIFT_KEYPOINT_ITEMS * (_featureNum + GlobalUtil::_texMaxDim)); // 11/19/2008
#else
  ...
  if(GlobalUtil::_DescriptorPPT)
  {
    ...
    if(GlobalUtil::_HalfSIFT)
      _descriptor_buffer.resize(64 * _featureNum + 16 * GlobalUtil::_texMaxDim); // 11/19/2008
    else
#endif // GPU_HESSIAN || GPU_SIFT_MODIFIED
...
void SiftPyramid::CopyFeatureVector(float *keys, float *descriptors)
{
  if(keys)
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
    memcpy(keys, &_keypoint_buffer[0], SIFT_KEYPOINT_ITEMS*_featureNum*sizeof(float));
#else
...
void SiftPyramid::SetKeypointList(int num, const float * keys, int run_on_current, int skip_orientation)
{
  ...
  /// copy the keypoints
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
  _keypoint_buffer.resize(num * SIFT_KEYPOINT_ITEMS);
  memcpy(&_keypoint_buffer[0], keys, SIFT_KEYPOINT_ITEMS * num * sizeof(float));
#else
...
void SiftPyramid::SaveSIFT(const char *szFileName)
{
  ...
  SiftKeypoint *pk = (SiftKeypoint *)&_keypoint_buffer[0];
#else
  ...
  // binary format (vlfeat) of output file:
  //  int VERSION (1) = "aff\1"
  //  int NUM_KEYPOINTS
  //  int DESC_LEN (128 for full sift, 64 for half sift)
  //  int WIDTH
  //  int HEIGHT
  // each keypoint:
  //   float x, y, scale
  //   float a11, a12, a21, a22 (orientation)
  //   int level+type(14b+2b)
  //   float response
  //   unsigned char desc[DESC_LEN]

  if(GlobalUtil::_BinarySIFT == 2)
  {
    std::ofstream out(szFileName, ios::binary);

    const char *magic = "aff\1";
    out.write(magic, 4);

    out.write((char *)&_featureNum, sizeof(int));

    int descLen = GlobalUtil::_DescriptorPPT ? descriptorSize : 0;
    out.write((char *)&descLen, sizeof(int));

    // image size
    out.write((char *)&_imageWidth, sizeof(int));
    out.write((char *)&_imageHeight, sizeof(int));

    float *pd = NULL;
	
    if(GlobalUtil::_DescriptorPPT)
      pd = &_descriptor_buffer[0];

    float fResponse = 0.0f;
    unsigned int fLevelAndType = 0;

    for (int i = 0; i < _featureNum; i++, pk++)
    {
      fResponse = pk->response;
      fLevelAndType = (pk->level << 2) | pk->type;

      // position x, y
      out.write((char *)(&pk->x), sizeof(float));
      out.write((char *)(&pk->y), sizeof(float));

      // scale
      float scale = pk->s * GlobalParam::_mrSize;
      out.write((char *)(&scale), sizeof(float));

      // orientation
      float fOrientation = pk->o;
      float a11 = cosf(fOrientation);
      float a12 = -sinf(fOrientation);
      float a21 = sinf(fOrientation);
      float a22 = cosf(fOrientation);

      out.write((char *)(&a11), sizeof(float));
      out.write((char *)(&a12), sizeof(float));
      out.write((char *)(&a21), sizeof(float));
      out.write((char *)(&a22), sizeof(float));

      // level is exported instead together with type (level 8b + type 2b)
      out.write((char *)(&fLevelAndType), sizeof(unsigned int));

      // hessian response
      out.write((char *)(&fResponse), sizeof(float));

      if(GlobalUtil::_DescriptorPPT)
      {
        for(int k = 0; k < descriptorSize; k++, pd++)
        {
          // descriptor should be always normalized -> GlobalUtil::_NormalizedSIFT = 1
          unsigned char outValue = (unsigned char)floor(0.5f+255.0f*(*pd));
          out.write((char *)(&outValue), sizeof(unsigned char));
        }
      }
    }
  }
  else
  {
    ...
    if(GlobalUtil::_BinarySIFT)
    {
      ...
      //  int DESC_LEN
      // each keypoint:
      //   float y, x, scale, orientation
      //   float response                   ... hessian or modified sift only
      //   unsigned short type              ... hessian or modified sift only
      //   unsigned short level             ... hessian or modified sift only
      //   float desc[DESC_LEN]

      std::ofstream out(szFileName, ios::binary);
      out.write((char* )(&_featureNum), sizeof(int));

      int dim = GlobalUtil::_DescriptorPPT ? descriptorSize : 0;
      out.write((char* )(&dim), sizeof(int));

      float *pd = GlobalUtil::_DescriptorPPT ? (&_descriptor_buffer[0]) : NULL;
      ...
      for(int i = 0; i < _featureNum; i++)
      {
        ...
        out.write((char* )(&pk->y), sizeof(float));
        out.write((char* )(&pk->x), sizeof(float));
        out.write((char* )(&pk->s), sizeof(float));
        out.write((char* )(&pk->o), sizeof(float));

        out.write((char* )(&pk->response), sizeof(float));

        out.write((char* )(&pk->type), sizeof(unsigned short));
        out.write((char* )(&pk->level), sizeof(unsigned short));
        ...
        if(GlobalUtil::_DescriptorPPT)
        {
          out.write((char* )(pd), descriptorSize * sizeof(float));
          pd += descriptorSize;
        }
      }
    }
    else // if(GlobalUtil::_BinarySIFT)
    {
      ...
      //   float y, x, scale, orientation
      //   float response                   ... hessian or modified sift only
      //   unsigned short type              ... hessian only
      //   unsigned short level             ... hessian only
      //   float desc[DESC_LEN]
      ...
      out << _featureNum << " " << dim << endl;

      for(int i = 0; i < _featureNum; i++)
      {
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
        // in y, x, scale, orientation order
        out << setprecision(2) << pk->y << " "
            << setprecision(2) << pk->x << " "
            << setprecision(3) << pk->s << " "
            << setprecision(3) << pk->o;

        out << " "
            << setprecision(8) << pk->response;
