							 ...
							 unsigned int gl_format, unsigned int gl_type )
{
	...
	if ((ws > _texMaxDim) || (hs > _texMaxDim))
	{
		if(simple_format)
		{
			...
			{
				std::cerr << "\nERROR: Input image is too big to fit into a texture. Try to use -ads option to automatic down-samping of input image to fit max texture dimension." << std::endl; 
				return 0;
			}
#endif // GPU_HESSIAN
