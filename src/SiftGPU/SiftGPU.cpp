...
inline void SiftGPU::InitSiftGPU()
{
  ...
  {
    std::cerr << "Can't initialize CUDA on a device with given ID: " << GlobalUtil::_DeviceIndex << ".\n";
    return;
  }
#else
  ...
  if(GlobalUtil::_verbose)
    ...
    std::cout << "\n[SiftGPU Language]:\t"
#endif // GPU_HESSIAN || GPU_SIFT_MODIFIED
              << (GlobalUtil::_UseCUDA ? "CUDA" : (GlobalUtil::_UseOpenCL ? "OpenCL" : "GLSL"))
              << "\n";
...
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
  _timing[11] = timer.GetElapsedTime();
#else
    _timing[10] = timer.GetElapsedTime();
#endif // GPU_HESSIAN
...
void SiftParam::ParseSiftParam()
{
...
#ifdef GPU_HESSIAN
  float dsigma0 = _sigma0 * sqrt(_sigmak*_sigmak - 1.0f);
 
  float sa = _sigma0 * powf(_sigmak, (float)_level_min);
  float sb = _sigman / powf(2.0f, (float)GlobalUtil::_octave_min_default);

  // _sigma_skip0 seems to be unused in the code
  ...
  {
#ifdef GPU_HESSIAN
    // change of gaussian sigma necessary to reach the next level 
    // for levels _min_level+1 ... _max_level
    _sigma[i-(_level_min+1)] = dsigma0 * powf(_sigmak, float(i-(_level_min+1)));
#else
...
void SiftGPUEX::DisplaySIFT()
{
  ...
  switch(_view)
  {
    ...
    case 4:
#ifdef GPU_HESSIAN
      DisplayPyramid(ShaderMan::UseShaderDisplayDOG, SiftPyramid::DATA_DOG, 0, 0);
#else
    ...
    case 6:
#ifdef GPU_HESSIAN
      DisplayPyramid(ShaderMan::UseShaderDisplayDOG, SiftPyramid::DATA_DOG, 1, 1);
      DisplayPyramid(ShaderMan::UseShaderDisplayKeypoints, SiftPyramid::DATA_KEYPOINT, 1, 1);
#else
...
void SiftGPU::PrintUsage()
{
  std::cout
    ...
    << "-o <string>         : Where to save SIFT features (works just for single input image)\n"
    ...
    << "-fo <int>         * : First octave to detect keypoints(default : 0)\n"
...
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
    << "-ads              * : Enabling auto-downsamping that try to fit max texture dimension (default: disabled).\n"
    << "-b                * : Write binary sift output file if specified. Contains additional info about\n"
	<< "                      keypoints (hesisan response, level and type). Text file is written by default.\n"
    << "-bvlf             * : Write binary output file (vlfeat format) if specified.\n"
    << "-half             * : Half sift - treat gradients with opposite signs as being the same if specified.\n"
#ifdef TOP_K_SELECTION
    << "-topk <int>       * : Limiting the overall number of features to a given number (distinct locations)\n"
...
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
      case MAKEINT3(a, d, s):
        GlobalUtil::_AutoImageDownScaling = 1;
        break;
      case MAKEINT4(b, v, l, f):
        GlobalUtil::_BinarySIFT = 2;
        break;
      case MAKEINT4(h, a, l, f):
        GlobalUtil::_HalfSIFT = true;
        break;
#endif // GPU_HESSIAN || GPU_SIFT_MODIFIED
      ...
      default:
        ...
        switch(opti)
        {
          ...
          case MAKEINT2(o, t): 
          {
            ...
            if(sscanf(param, "%f", &factor) && (factor > 0))
            {
              GlobalUtil::_MultiOrientationThreshold = factor;
              i++;
            }
            break;
          }
          ...
          case MAKEINT2(f, o): 
          {
            int first_octave = -3;
#ifdef GPU_HESSIAN
            if(sscanf(param, "%d", &first_octave) && (first_octave >= 0))
#else
          ...
          // method to softly limit the number of detected features
          case MAKEINT3(t, c, 2):     // -tc2    keep the highest level (should be faster than -tc) - downward
          case MAKEINT3(t, c, 3):     // -tc3    keep the lowest levels
          case MAKEINT2(t, c):        // -tc, -tc1, keep the highest levels
          case MAKEINT3(t, c, 1):     //
#if (defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED) && defined TOP_K_SELECTION
          case MAKEINT4(t, o, p, k):  // -topk   top k selection
#endif // (GPU_HESSIAN || GPU_SIFT_MODIFIED) && TOP_K_SELECTION
          {
            switch (opti)
            {
              case MAKEINT3(t, c, 2):
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
                GlobalUtil::_TruncateMethod = SiftPyramid::TRUNCATE_METHOD_KEEP_HIGHEST_LEVELS_1; // -tc2    keep the highest level (should be faster than -tc)
#else
              ...
              case MAKEINT3(t, c, 3):
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
                GlobalUtil::_TruncateMethod = SiftPyramid::TRUNCATE_METHOD_KEEP_LOWEST_LEVELS; // -tc3    keep the lowest levels
#else
                GlobalUtil::_TruncateMethod = 2;
#endif // GPU_HESSIAN || GPU_SIFT_MODIFIED
                break;
#if (defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED) && defined TOP_K_SELECTION
              case MAKEINT4(t, o, p, k): // top k
                GlobalUtil::_TruncateMethod = SiftPyramid::TRUNCATE_METHOD_TOP_K; // -topk   top k selection
                break;
#endif // (GPU_HESSIAN || GPU_SIFT_MODIFIED) && TOP_K_SELECTION
              default:
#if defined GPU_HESSIAN || defined GPU_SIFT_MODIFIED
                GlobalUtil::_TruncateMethod = SiftPyramid::TRUNCATE_METHOD_KEEP_HIGHEST_LEVELS_0; // -tc, -tc1, keep the highest levels
#else
...
float SiftParam::GetLevelSigma(int level)
{
  return _sigma0 * powf(2.0f,  float(level) / float(_dog_level_num)); //bug fix 9/12/2007
}
