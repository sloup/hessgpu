...
CuTexImage::CuTexImage(int width, int height, int nchannel, GLuint pbo)
{
  ...
  // check size of pbo
  GLint bsize;
  GLint esize = width * height * nchannel * sizeof(float);
...
void CuTexImage::ReInitTexture(int width, int height, int nchannel, void *data)
{
  int size;
...
#ifdef _DEBUG
	ProgramCU::CheckErrorCUDA("CuTexImage::ReInitTexture");
#endif
}
